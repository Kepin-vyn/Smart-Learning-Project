...
export default function SummaryPage() {
  ...
  const { result, lang } = useLearningStore()
    ...
    return (
      <main className="min-h-screen flex items-center justify-center p-6" style={{ background: 'var(--color-bg)' }}>
        <div className="text-center p-12 rounded-[2rem] max-w-md w-full shadow-card border"
          ...
          <h1 className="text-xl font-bold mb-4" style={{ fontFamily: 'var(--font-heading)', color: 'var(--color-text)' }}>{t.summary_no_session_title}</h1>
      ...
      <main className="max-w-5xl mx-auto px-6 py-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-5 items-start mb-8">
          ...
          <div className="md:col-span-7 rounded-2xl md:rounded-[2.5rem] p-6 md:p-8 flex flex-col items-center justify-center border-2 relative overflow-hidden group shadow-lifted"
            ...
            <span className="text-xs font-bold uppercase tracking-widest mb-4" style={{ color: 'var(--color-on-secondary-container)' }}>{t.summary_result_label}</span>
            ...
            <p className="font-semibold mb-6 text-base md:text-lg text-center" style={{ color: 'var(--color-on-secondary-container)', opacity: 0.8 }}>
              {t.summary_score_label(session.score)} {session.score >= 80 ? '🌟' : '⭐'}
            </p>
            ...
            <div className="flex items-center gap-8 mt-6">
              ...
              <div className="text-center">
                <div className="text-sm md:text-base font-bold"
                  ...
                  {isPass ? t.summary_pass : t.summary_review}
                </div>
                <div className="text-xs uppercase tracking-wide mt-1" style={{ color: 'var(--color-text-subtle)' }}>{t.summary_status_label}</div>
              </div>
            </div>
          </div>
          ...
          <div className="md:col-span-5 rounded-2xl md:rounded-[2.5rem] p-5 md:p-7 border-2 shadow-soft"
            ...
            <div className="flex items-center gap-3 mb-5">
              ...
              <h2 className="font-bold text-base md:text-lg" style={{ fontFamily: 'var(--font-heading)', color: 'var(--color-text)' }}>{t.summary_review_title}</h2>
            </div>

            {session.missedQuestions.length > 0 ? (
              <div className="space-y-3">
                <p className="text-xs md:text-sm mb-2" style={{ color: 'var(--color-text-muted)' }}>{t.summary_missed_intro}</p>
                {session.missedQuestions.map((mq, i) => (
                  <div key={i} className="p-3 md:p-4 rounded-2xl md:rounded-3xl border flex items-start gap-3 md:gap-4 transition-transform duration-300 hover:translate-x-2"
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[10px] md:text-xs font-semibold mb-1" style={{ color: 'var(--color-primary)' }}>{t.summary_answer_label(mq.correctAnswer)}</p>
              ...
              <div className="text-center py-6">
                ...
                <p className="font-semibold text-sm md:text-base" style={{ color: 'var(--color-primary)' }}>{t.summary_all_correct}</p>
                <p className="text-xs md:text-sm mt-1" style={{ color: 'var(--color-text-subtle)' }}>{t.summary_all_correct_sub}</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row justify-center items-stretch sm:items-center gap-3 md:gap-4 mb-8 md:mb-10 px-4">
          {!isPass && (
            <button onClick={() => router.push('/steps')}
              ...
              <span className="material-symbols-outlined text-lg md:text-xl">refresh</span> {t.summary_btn_retry}
            </button>
          )}
          <button onClick={() => router.push('/history')}
            ...
            <span className="material-symbols-outlined text-lg md:text-xl">history</span> {t.summary_btn_history}
          </button>
          <button onClick={() => router.push('/')}
            ...
            <span className="material-symbols-outlined text-lg md:text-xl filled">play_arrow</span> {t.summary_btn_new}
          </button>
        </div>
