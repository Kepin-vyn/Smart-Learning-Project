    ...
    <div className="min-h-screen" style={{ background: 'var(--color-bg)' }}>
      ...
      <main className="max-w-5xl mx-auto px-6 py-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-5 items-start mb-8">
          ...
          {/* Score card */}
          <div className="md:col-span-7 rounded-[2.5rem] p-8 flex flex-col items-center justify-center border-2 relative overflow-hidden group shadow-lifted"
            ...
            <div className="flex items-end gap-2 mb-2">
              <span className="font-bold leading-none" style={{ fontSize: '5rem', fontFamily: 'var(--font-heading)', color: 'var(--color-secondary)' }}>
                {session.score}
              </span>
              <span className="text-2xl mb-3 opacity-50" style={{ color: 'var(--color-secondary)' }}>/100</span>
              <span className="text-4xl mb-2 anim-bounce-gentle">{session.score >= 80 ? '🌟' : session.score >= 60 ? '⭐' : '🌱'}</span>
            </div>
            <p className="font-semibold mb-6 text-lg" style={{ color: 'var(--color-on-secondary-container)', opacity: 0.8 }}>
            ...
            <div className="flex items-center gap-8 mt-6">
              <div className="text-center">
                <div className="text-2xl font-bold" style={{ fontFamily: 'var(--font-heading)', color: 'var(--color-secondary)' }}>{session.totalSteps}</div>
              ...
              <div className="text-center">
                <div className={`text-base font-bold ${isPass ? '' : ''}`}
          ...
          {/* Review section */}
          <div className="md:col-span-5 rounded-[2.5rem] p-7 border-2 shadow-soft"
            ...
            <div className="flex items-center gap-3 mb-5">
              ...
              <h2 className="font-bold text-lg" style={{ fontFamily: 'var(--font-heading)', color: 'var(--color-text)' }}>Review Materi</h2>
            </div>

            {session.missedQuestions.length > 0 ? (
              <div className="space-y-3">
                <p className="text-sm mb-3" style={{ color: 'var(--color-text-muted)' }}>Soal yang perlu diulang:</p>
                {session.missedQuestions.map((mq, i) => (
                  <div key={i} className="p-4 rounded-3xl border flex items-start gap-4 transition-transform duration-300 hover:translate-x-2"
                    ...
                    <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0"
                      ...
                      <span className="material-symbols-outlined text-base filled" style={{ color: 'var(--color-primary)' }}>bookmark</span>
                    </div>
                    <div className="min-w-0">
                      <p className="text-sm font-medium mb-1" style={{ color: 'var(--color-text)' }}>{mq.question}</p>
                      <p className="text-xs font-semibold mb-1" style={{ color: 'var(--color-primary)' }}>Jawaban: {mq.correctAnswer}</p>
                      <p className="text-xs leading-relaxed" style={{ color: 'var(--color-text-muted)' }}>{mq.explanation}</p>
                    </div>
                  </div>
                ))}
                <div className="flex items-center gap-2 p-3 rounded-2xl border mt-2"
                  style={{ background: 'rgba(66,98,117,0.08)', borderColor: 'rgba(134,167,187,0.25)' }}>
                  <span className="material-symbols-outlined text-sm" style={{ color: 'var(--color-tertiary)' }}>info</span>
              ...
              <div className="text-center py-6">
                ...
                <p className="font-semibold" style={{ color: 'var(--color-primary)' }}>Semua soal dijawab dengan benar!</p>
                <p className="text-sm mt-1" style={{ color: 'var(--color-text-subtle)' }}>Luar biasa, kamu sudah sangat paham 🌿</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row justify-center items-center gap-4 mb-10">
          {!isPass && (
            <button onClick={() => router.push('/steps')}
              className="w-full md:w-auto px-10 py-4 rounded-full font-semibold border-2 flex items-center justify-center gap-2 squishy-btn shadow-soft hover:bg-opacity-80 transition-all"
              style={{ background: 'white', borderColor: 'var(--color-secondary-container)', color: 'var(--color-secondary)' }}>
              <span className="material-symbols-outlined">refresh</span> Ulangi Sesi
            </button>
          )}
          <button onClick={() => router.push('/history')}
            className="w-full md:w-auto px-10 py-4 rounded-full font-semibold border-2 flex items-center justify-center gap-2 squishy-btn shadow-soft hover:opacity-90 transition-all"
            style={{ background: 'white', borderColor: 'var(--color-primary-container)', color: 'var(--color-primary)' }}>
            <span className="material-symbols-outlined">history</span> Lihat Riwayat
          </button>
          <button onClick={() => router.push('/')}
            className="w-full md:w-auto px-10 py-4 rounded-full font-semibold text-white flex items-center justify-center gap-2 squishy-btn shadow-lifted hover:scale-105 transition-all"
            style={{ background: 'var(--color-primary)', boxShadow: '0 8px 24px rgba(76,100,85,0.25)' }}>
            <span className="material-symbols-outlined filled">play_arrow</span> Materi Baru
          </button>
        </div>
